...
| RISK-5 | Command-policy bypass lets autopilot run arbitrary commands | Medium | High | Execute build/test as **argv, no shell**; reject shell metacharacters; **flag-denylist** (`-EncodedCommand`/`-e`/`-Command`/`-c`, `npx`, `pip install`, `npm install`/`run`) + absolute-path launcher resolution in `launch.ps1`; trusted prefixes + coarse filter in schema **outside** agent-editable `.autopilot.json`; config read once at launch; test mutated-config + base64 bypass rejection. Residual: build/test inherently execute trusted project scripts — isolation (container) + agent safety rules + human PR review are the backstop, not the regex | 1.2, 2.1, 5.1 |
...
- [x] 2.1 `scripts/autopilot/launch.ps1`: `#requires -Version 7.0`; load + draft-07-validate `.autopilot.json` (`Test-Json -SchemaFile`); **enforce command policy** (re-check trusted prefixes, **tokenize `build`/`test` into argv**, reject shell metacharacters `; & | $ \` < > ( ) { } " ' % newline`, and apply a **flag denylist**: reject `pwsh`/`powershell` `-EncodedCommand`/`-e*`/`-Command`/`-c*`, `npx`, `pip install`, `npm install`/`npm run`, and resolve launchers to absolute paths / sanitize `PATH` (no repo dir on `PATH`); never `Invoke-Expression`/`bash -c`); **config is read once at launch and not re-read mid-run**; **canonicalize `planPath` against repo root, reject escapes**; docker pre-flight when `-Mode container` (**fail loudly**, no host fallback); sweep stale secret files; dispatch (REQ-1, REQ-2, RISK-2, RISK-5) [after: 1.2] `L`
...
- [ ] 5.1 Pester: draft-07 schema validation (valid/invalid); **command-policy rejection** (shell metacharacters; **`pwsh -e <base64>` / `-EncodedCommand` / `-Command` and `npx`/`pip install` forms rejected by the launcher flag-denylist**; mutated-`.autopilot.json` that adds a command → rejected; `planPath` traversal/absolute → rejected); mode dispatch; container-no-docker fails loudly (REQ-9, RISK-5, RISK-2) [after: 2.1] `M`
