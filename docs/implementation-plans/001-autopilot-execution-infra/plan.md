...
- [ ] 1.1 Add `.autopilot.json` at repo root using the **design-note shape**: `runtime`, `build` + `test` (single strings), `timeout`, `planPath`, `copilotAuth.{credentialTarget}`, `gitProvider`, `gitAuth`, `model`, `git.{name,email}`, `maxIterationsPerStep`, `mode` default (REQ-1) `S`
