...
## Verification & Close-out (Phase 4)

**Trigger:** Phase 4 (design notes + integrity/verification) run as a separate manual pass after the container committed Phases 1–3 (`57365a7`, `f1cc71d`, `bf562ab`). The container left Phase 4 unchecked and several deferred integrity outputs uncommitted.

**Gaps found during verification (and fixed):**
1. **Registry stale.** `plugin.json` was committed (1.6) but the deferred 4.4 `Build-Registry.ps1` regen never ran — `registry.json` (and the generated `README.md` plugin table) lacked the new bundle files/hashes. Regenerated.
2. **Dogfood never synced.** `.github/skills/autopilot/**` did not exist; `Sync-Dogfood.ps1 -WhatIf` reported 19-file drift. Ran `Sync-Dogfood.ps1` → 0 drift (also synced agent rule #10 + `/ci` 3-option menu into `.github/`).
3. **`.autopilot.json` still tracked.** Step 2.3 added the gitignore entry but never untracked the repo's committed config (from Plan 001). `git rm --cached .autopilot.json` (kept on disk) — satisfies REQ-11.
4. **PSScriptAnalyzer regressions** on changed/new scripts: `host-command.ps1` (4 aligned-`=` whitespace, new file), `launch-host.ps1` (`Quote-TokenForCmd`/`Quote-TokenForPowerShell` unapproved verb → renamed `ConvertTo-CmdQuotedToken`/`ConvertTo-PowerShellQuotedToken`), `launch.ps1` (em-dash → BOM warning, fixed; pre-existing `$dispatchParams` alignment also de-aligned). Changed/new scripts now zero-warning; pre-existing warnings in unchanged moved files (`launch-sandbox.ps1`, `clean-sandbox-cache.ps1`, `get-credential.ps1`, `validate-auth.ps1`) are out of scope.
5. **Stale design-note config.** `autopilot-execution.design.md` Configuration block carried a stale `planPath: 002-plugin-registry/…` placeholder + `copilotAuth: { credentialTarget }` object shape; the real schema has no `planPath` and `copilotAuth` is a string enum. Rewritten to match `.autopilot.json.example`; documented that the plan path is derived from `-PlanSlug`.

**4.1** new `autopilot-skill.design.md`. **4.2** `autopilot-execution.design.md` — globs, `/ci`→skill flow, Custom Host Launch Command + `AUTOPILOT_DISABLE_HOST` + first-run bootstrap sections, `host-command.ps1` inventory row, fixed Configuration. **4.3** `plugin-registry.design.md` self-contained narrative (already present), `copilot-customizations.design.md` inventory + 3-option `/ci` flow, `.design-notes.md` index row + globs.

**Verification results:** `validate.ps1` green; `Test-Registry.ps1` green; `Sync-Dogfood.ps1 -WhatIf` 0 drift; PSScriptAnalyzer zero-warning on changed/new scripts; `Invoke-Pester ./tests/autopilot` 7/7; `docker build` of the relocated devcontainer succeeded (`COPY scripts/container-entrypoint.sh` resolves under the `.github/skills/autopilot` context — RISK-7); launch.ps1 guards confirmed (`.autopilot.json` presence + `AUTOPILOT_DISABLE_HOST`); agent Absolute Rule #10 present (rule 9 intact); `/ci` carries the "**Read** `.github/skills/autopilot/SKILL.md`" imperative; grep clean for `Initialize-Autopilot`/`scripts/autopilot`/`.devcontainer/autopilot` outside the plan. All 18 REQs + 8 RISKs satisfied (RISK-1/RISK-2 live host/sandbox dry-runs not executed — repo-root resolution unchanged + Pester-covered + static review).
