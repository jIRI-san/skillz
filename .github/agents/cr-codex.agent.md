...
- Null or missing value dereferences not guarded
- Missing error handling: exceptions not caught where they should be, errors not propagated correctly
...
- Performance: N+1 query patterns, synchronous I/O on hot paths, unbounded collection growth, resource leaks, undisposed `IDisposable`
- Concurrency: shared mutable state without synchronization, thread-unsafe collections, deadlock risks
...
- Inheritance used where composition is appropriate; reflection or unsafe type handling where strongly-typed approaches exist
...
2. Load design notes relevant to the changed subsystem to understand expected behavior contracts before reviewing

## Output Format

Start with `## Findings (Correctness)`. For each issue use this structure:
...
If no issues found in your scope: output `## Findings (Correctness)` followed by `None.`
